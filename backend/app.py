...
PDF_LANGUAGES = ["Japanese", "Hindi", "Russian", "Polish", "Arabic", "German"]
...
# to the new `auth_tokens` map (token -> expiry_iso).
def load_users():
    global USERS
...
# ==========================
# PDF upload route with dedupe by name & hash + language
@app.route("/api/upload-pdf", methods=["POST"])
@require_auth
def upload_pdf():
    NAME_SIMILARITY_THRESHOLD = float(os.environ.get("NAME_SIMILARITY_THRESHOLD", NAME_SIMILARITY_DEFAULT))

    # read language from form (optional but validated)
    language = (request.form.get("language") or "").strip()
    if language and language not in PDF_LANGUAGES:
...
# NEW: delete a PDF uploaded by the current user
@app.route("/api/my-pdfs/<pdf_id>", methods=["DELETE"])
@require_auth
def delete_my_pdf(pdf_id):
    user = request.current_user
    ...
    entry = meta[idx]
    if entry.get("uploaded_by") != username:
        return jsonify({"error": "Forbidden - not your upload"}), 403
    ...
    if path and os.path.exists(path):
        try:
            ...
            print(f"[DELETE-MY-PDF] Failed to remove file {path}: {e}")

    # Remove from global meta
    del meta[idx]
    save_global_pdfs(meta)

    # Remove from user's global_uploads list if present
    user_data = USERS.get(user_id)
    if user_data is not None:
        uploads = user_data.get("global_uploads", [])
        if isinstance(uploads, list):
            uploads = [u for u in uploads if u.get("id") != pdf_id]
            user_data["global_uploads"] = uploads
            save_users()
...
# NEW: admin export accepted PDFs (all users or specific user, with optional date range)
@app.route("/api/admin/export-accepted", methods=["GET"])
@require_role("admin")
def admin_export_accepted():
    """
    Export accepted PDFs as a ZIP.

    Query params:
      - username   (optional) -> filter by uploaded_by
      - from_date  (optional, YYYY-MM-DD) inclusive
    ...
    # Filter accepted PDFs
    items = [e for e in meta if e.get("status") == "Accepted"]

    # Filter by uploader if provided
    if username:
        items = [e for e in items if e.get("uploaded_by") == username]
    ...
    from_d = parse_date_only(from_date) if from_date else None
    to_d = parse_date_only(to_date) if to_date else None

    # Filter by uploaded_at date (if provided)
    if from_d or to_d:
        filtered = []
        for e in items:
            uploaded_at = e.get("uploaded_at")
            if not uploaded_at:
                continue
            try:
                up_dt = datetime.fromisoformat(uploaded_at).date()
            except Exception:
                # skip invalid timestamps
                continue
            if from_d and up_dt < from_d:
                continue
            if to_d and up_dt > to_d:
                continue
            filtered.append(e)
        items = filtered
    ...
    if not items:
        if username:
            return jsonify({"error": f"No accepted PDFs found for user '{username}' with given filters"}), 400
        else:
            return jsonify({"error": "No accepted PDFs found with given filters"}), 400
    ...
    try:
        ...
        # Name zip according to scope
        if username:
            zip_name = f"accepted_pdfs_{username}.zip"
        else:
            zip_name = "accepted_pdfs_all_users.zip"

        return send_file(tmp_name, as_attachment=True, download_name=zip_name)
    except Exception as e:
